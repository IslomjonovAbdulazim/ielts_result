...
const IeltsResultsApp = () => {
    ...
    useEffect(() => {
        ...
        if (!code) {
            const noCodeError = new Error('No session code found in URL');
...
document.addEventListener('DOMContentLoaded', () => {
    logger.info('APP', 'DOM loaded, initializing React app');
    
    const root = ReactDOM.createRoot(document.getElementById('root'));
    
    root.render(
        React.createElement(ErrorBoundary, null,
            React.createElement(IeltsResultsApp)
        )
    );
    
    logger.info('APP', 'React app rendered successfully');
});