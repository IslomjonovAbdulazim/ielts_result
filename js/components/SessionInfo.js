// SessionInfo.js - Session information display component
const SessionInfo = ({ data }) => {
    ...
    const infoCards = [
        ...
        {
            title: 'Status',
            value: session_info?.status ? getStatusBadge(session_info.status) : 'N/A'
        },
        {
            title: 'Duration',
            ...
            <div className="session-header">
                {infoCards.map((card, index) => (
                    <div key={index} className="info-card">
                        <h3>{card.title}</h3>
                        <p>
                            {typeof card.value === 'string' && card.value.includes('<span') ? 
                                <span dangerouslySetInnerHTML={{ __html: card.value }} /> :
                                card.value
                            }
                        </p>
                    </div>
